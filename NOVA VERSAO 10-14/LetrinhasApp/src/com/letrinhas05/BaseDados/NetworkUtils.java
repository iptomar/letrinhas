...
public class NetworkUtils {
	...
	public static byte[] getFile(final String url) {
		...
		// *****************************************
		//getRequest.addHeader("Autorizacao", "Basic "+ base64);
		try {
	...
	/**
	 * Envia um resultado do teste para um deterninado URL Usa uma biblioteca da
	 * Apache: http://james.apache.org/download.cgi#Apache_Mime4J
	 * 
	 * @param url
	 *            O url de destino para enviar os resultados
	 * @param correcaoTeste
	 *            Um objecto correcao de teste com os dados carregados que se
	 *            pretendem enviar
	 * 
	...
	public static boolean postResultados(final String url,
			CorrecaoTeste correcaoTeste) {
		AndroidHttpClient client = AndroidHttpClient.newInstance("letrinhas");
		...
		// *****************************************
		//postRequest.addHeader("Autorizacao", "Basic "+ base64);
		...
		if (correcaoTeste instanceof CorrecaoTesteLeitura) {
			...
			try {
				builder.addPart("details",
						new StringBody(detalhes, Charset.forName("UTF-8")));
			...
			// // Envia Um ficheiro////
			builder.addBinaryBody("audio",
					Utils.getFileSD2(teste.getAudiourl()),
					ContentType.APPLICATION_OCTET_STREAM, "cenas.3gp");
		} else {
		...
		try {
			Log.d("Letrinhas",
					"Enviando um http request. para os resultados do teste");
