...
public class Teste_Poema_Prof extends Activity {
	...
	// flags para verificar os diversos estados do teste
			...
			private String filePath, texto, pastas, fileName;
...
//				}
				...
				gravador.setOutputFile(filePath);
			...
			private void escutaBotoes() {
				...
				cancelar.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View view) {
						// salta a avaliação e vai para o próximo teste 
						// elimina a gravação
						File file = new File(filePath);
						if (file.exists()) {
							file.delete();
			...
			private void startPlay() {
				if (!playing) {
					...
					try {
						...
						reprodutor.setDataSource(filePath);
						reprodutor.prepare();
						reprodutor.start();
			...
			private void startAvalia() {
				if (modo) { // se estï¿½ em modo de professor
					...
					if (file.exists()) { // Verifica se jï¿½ fez uma gravaï¿½ï¿½o
						...
						file= new File(filePath.substring(0, filePath.length()-5)+"_aval.txt");
